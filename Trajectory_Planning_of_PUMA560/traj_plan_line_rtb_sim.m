%% 计算直线插补序列并画图展示
clear,clc,close all;
P1=[0.452,  -0.150,  0.432,       0,     0,    0  ];
P2=[0.232,  -0.078,  0.810,    53.4,  -1.9,   34.6];
% P1=[0.242,  -0.676,  0.137,   -65.4,  20.7,  37.6];
% P2=[0.299,  -0.026,  0.596,  -155.7,  -6.3,  36.5];
N=48; % 插值点的个数 
P= traj_plan_line(P1,P2,N);

%% 全部P点逆运动学的求解和选择
th(N+2,6)=0;% 六关节角解序列初始化
...
subplot(3,2,4), plot(t,th(:,4),'r'),xlabel('t'),ylabel('q4');grid on; 
...
subplot(3,2,4), plot(t(1:end-1),diff(th(:,4))./diff(t),'g'),xlabel('t'),ylabel('qd4');grid on; 
...
subplot(3,2,4), plot(t(1:end-2),diff(diff(th(:,4))./diff(t))./diff(t),'b'),xlabel('t'),ylabel('qdd4');grid on; 
